...
if ($format === 'png') {
    ...
    // Método 1: binario de ImageMagick (magick/convert) — aislado, no crashea Apache
    $magickBin = null;
    foreach (['/usr/local/bin/magick', '/usr/local/bin/convert', '/opt/homebrew/bin/magick', 'magick', 'convert'] as $bin) {
        if (@is_executable($bin) || $bin === 'magick' || $bin === 'convert') {
            $magickBin = $bin;
            break;
        }
    }
    ...
    if ($magickBin) {
        $tmpPng = tempnam(sys_get_temp_dir(), 'logo_') . '.png';
        // Comando: convierte el SVG a PNG del ancho pedido, fondo transparente
        $cmd = escapeshellarg($magickBin) . ' -background none -density 300 '
        ...
        if (file_exists($tmpPng) && filesize($tmpPng) > 0) {
            ...
            @unlink($tmpPng);
            error_log('PNG via binary failed for pid ' . $pid . ': ' . $output);
        }
    }

    // Método 2 (fallback): extensión Imagick, solo si el binario no existe
    if ($pngData === null && extension_loaded('imagick')) {
        try {
            ...
            $im->resizeImage((int)$size, 0, Imagick::FILTER_LANCZOS, 1);
            $pngData = $im->getImageBlob();
            $im->clear();
            $im->destroy();
        } catch (Throwable $e) {
            error_log('PNG via Imagick failed for pid ' . $pid . ': ' . $e->getMessage());
            $pngData = null;
        }
    }

    // Servir el PNG si se generó
    ...
    // Si nada funcionó, error claro (no página vacía)
    dlError(503, 'png_unavailable', 'PNG generation failed on this server. Please try the SVG.');
}
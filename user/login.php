...
// ── Redirect seguro tras login ──
function getSafeRedirect($setting)
{
    $target = $_REQUEST['redirect'] ?? '';
    $base   = $setting['website_url'];
    // Solo permitir redirects dentro del propio sitio
    if (!empty($target) && strpos($target, $base) === 0) {
        return $target;
    }
    return $base . '/user/';
}
$redirectTo = getSafeRedirect($setting);

// Guardar el redirect al llegar por GET, para usarlo tras el POST
if (!empty($_GET['redirect'])) {
    $_SESSION['login_redirect'] = getSafeRedirect($setting);
}
$redirectTo = $_SESSION['login_redirect'] ?? ($setting['website_url'] . '/user/');
...
// Ya está logueado
if ($user->is_loggedin()) {
    header('Location: ' . ($_SESSION['login_redirect'] ?? $setting['website_url'] . '/user/'));
    ...
    <script>
        ...
        if (digits.length) {
            ...
            function updateCodeHidden() {
                var code = Array.from(digits).map(function(d) {
                    return d.value;
                }).join('');

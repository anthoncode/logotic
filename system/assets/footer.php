...
<script>
  ...
  bookmarkBtn.addEventListener('click', function() {
    ...
    this.addEventListener('animationend', () => this.classList.remove('bouncing'), {
      once: true
    });
  });
  ...
  function toggleFav(id, el) {
    ...
    if (favs.has(id)) {
      ...
      showToast('Added to favourites \u2605', 'Saved');
    }
  }
  ...
  document.addEventListener('click', e => {
    if (!e.target.closest('.hero-search')) suggestions.classList.remove('show')
  });
...
<style>
  ...
  .fav-float:not(.has-items) .fav-float-count {
    display: none;
  }
  ...
  .fav-float-count.bounce {
    animation: favBounce .5s ease;
  }

...
<style>
    ...
    .cover-upload-zone.has-image .cover-placeholder {
        display: none;
    }

    .cover-upload-zone.has-image img {
        display: block;
        margin-top: 0;
    }
    ...
    <div class="adm-page-header">
        ...
        <div style="margin-left:auto; display:flex; gap:.5rem;">
            <a href="<?php echo $setting['website_url']; ?>/page/<?php echo htmlspecialchars($v['slug']); ?>/"
                ...
                style="<?php echo ($isEdit && !empty($v['slug'])) ? '' : 'display:none;'; ?>">
...
<script>
    ...
    document.getElementById('coverInput').addEventListener('change', function() {
        ...
        if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
            showAlert('error', 'Only JPG, PNG or WebP.');
            this.value = '';
            return;
        }
    ...
    function savePage() {
        ...
        if (!slug) {
            showAlert('error', 'Slug is required.');
            return;
        }
        if (!content) {
        ...
        fetch('<?php echo $setting['website_url']; ?>/admin/ajax-save-page.php', {
            ...
            .then(function(data) {
                ...
                if (data.success) {
                    ...
                    // Mostrar/actualizar el botón "View" (ojo) con el slug guardado
                    var viewBtn = document.getElementById('viewPageBtn');
                    if (viewBtn) {
                        viewBtn.href = '<?php echo $setting['website_url']; ?>/page/' + data.slug + '/';
                        viewBtn.style.display = '';
                    }
                    status.className = 'save-status saved';
                    status.innerHTML = '<i class="fa-solid fa-circle-check"></i> Saved ' + new Date().toLocaleTimeString();
                    showAlert('success', data.message);
                    // Enlace "Ver página" (se abre en nueva pestaña)
                    var pageUrl = '<?php echo $setting['website_url']; ?>/page/' + data.slug + '/';
                    var viewMsg = data.message +
                        ' &nbsp;<a href="' + pageUrl + '" target="_blank" rel="noopener" ' +
                        'style="color:var(--adm-accent);font-weight:700;text-decoration:none;">' +
                        '<i class="fa-solid fa-arrow-up-right-from-square"></i> View page</a>';
                    showAlert('success', viewMsg);

...
if (isset($_REQUEST['id'])) {
	if (!empty($_REQUEST['id'])) {
		$details = $product->catdetails(trim($_REQUEST['id']));
	} else {
		header("location: categories.php");
	}
} else {
	$details = $product->catdetails('0');
}
if (isset($_REQUEST['id']) && 'delete' == @$_REQUEST['action']) {
	if (!empty($_REQUEST['id'])) {

		$result = $product->removecat($_REQUEST['id']);
		unset($details);
	}
}
...
?>
	...
	<div class="content">
		...
		<div class="my-3 p-3 bg-white rounded box-shadow">
			<h3>Are you sure you want to delete this category?</h3><br>
			<form action="remove-category.php?id=<?php echo $details['id']; ?>" method="post" class="form-horizontal">

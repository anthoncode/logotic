...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['account']) && $_POST['account'] === 'cpassword') {
        ...
        if (empty($errors)) {
            ...
            if (empty($errors)) {
                // Verificar contraseña actual con el nuevo método (bcrypt/MD5)
                if (!$auth->verifyCurrentPassword($_SESSION['uid'], $currentPwd)) {
                    $errors[] = 'Current password is incorrect.';
                } else {
                    // Guardar nueva contraseña en bcrypt
                    $auth->changePassword($_SESSION['uid'], $newPwd);
                    $success = 'password';
                }
            }
        }
